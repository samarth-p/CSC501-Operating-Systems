...
void allocate_page_dir(int pid)
{
	int j, frame;	
	pd_t *page_dir;
	...
	proctab[pid].pdbr = (frame + FRAME0) * NBPG;
	frm_tab[frame].fr_status = FRM_MAPPED;
	frm_tab[frame].fr_pid = pid;
	frm_tab[frame].fr_type = FR_DIR;
	
	page_dir = (FRAME0 + frame) * NBPG;
	
	for(j=0; j < 1024; j++){
		page_dir[j].pd_write = 1;

		if (j < 4) {
			page_dir[j].pd_pres = 1;
			page_dir[j].pd_base = FRAME0 + j;
		}
	}
}
