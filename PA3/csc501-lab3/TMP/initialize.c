...
struct fifo_queue * fifohead = NULL;
int num_sc = 0;
int sc_curr = 0;
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	for(i=0; i < 4; i++){
		...
		frm_tab[frame].fr_refcnt = 1024;
		pt_entry = (pt_t *)((FRAME0 + frame) * NBPG);
	...
	create_page_dir(NULLPROC, &pd_frame);
	init_page_dir(pd_frame);

	pptr->pdbr = (FRAME0 + pd_frame) * NBPG ;
	write_cr3(pptr->pdbr);
