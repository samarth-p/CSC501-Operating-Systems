...
int	resched()
{
	...
	unsigned int pp,qq,bs_num,page_offset;
	pd_t *pd_entry;
	pt_t *pt_entry;
	unsigned long pdbr, pt;
	virt_addr_t v_addr;
	for(i=0; i < NFRAMES; i++){
		...
		get_virt_addr(frm_tab[i].fr_vpno * NBPG, &v_addr);        
		pp = v_addr.pd_offset;
		qq = v_addr.pt_offset; 
		...
		pd_entry = (pd_t *) pt;
		pt_entry = (pt_t *) (pd_entry->pd_base * NBPG + sizeof(pt_t) * qq );       

		if(pt_entry->pt_dirty){
			if(bsm_lookup(currpid, frm_tab[i].fr_vpno * NBPG, &bs_num, &page_offset) == SYSERR)
				kill(currpid);

			if(bsm_tab[bs_num].bs_ispriv == BSM_SHARED)
				write_bs((char *)(pt_entry->pt_base * NBPG), bs_num, page_offset);
	...
	for(i=0; i <NFRAMES; i++){
		...
		if(bsm_tab[bs_num].bs_ispriv == BSM_SHARED)
			read_bs((char*)((FRAME0 + i)*NBPG), bs_num, page_offset);
